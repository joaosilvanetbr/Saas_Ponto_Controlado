export default function Button({ children, variant = 'filled', size = 'md', onClick, disabled, fullWidth, style }) {
  const s = styles(variant, size, fullWidth, disabled)
...
function styles(variant, size, fullWidth, disabled) {
  const sizeMap = {
  ...
  const variantMap = {
    ...
    text: { background: 'transparent', color: 'var(--color-accent)', paddingInline: '12px' },
  }
