...
import type { ConfigUsuario, Ponto, Marcacao, Ponto as PontoType } from '../types'
...
const TIPOS = {
  registro: { label: 'Registro', emoji: '✅' },
  falta: { label: 'Falta', emoji: '❌' },
  feriado: { label: 'Feriado', emoji: '🎉' },
  ferias: { label: 'Férias', emoji: '🏖️' },
  extra_pago: { label: 'Extra Pago', emoji: '💰' },
  extra_banco: { label: 'Extra Banco', emoji: '⏰' },
}
...
export default function HistoricoPage() {
  ...
  function calcularHorasPonto(ponto: PontoType): number {
  ...
  function calcularSaldoPonto(ponto: PontoType): number {
    ...
    return calcularSaldoDia(ponto, config.jornadaMinutos, config.intervaloMinutos || 0)
  }
  ...
  const [detalhe, setDetalhe] = useState<Ponto | null>(null)
...
interface PontoAcumulado extends Ponto {
  ...
  const pontosComAcumulado = useMemo(() => {
    return pontosDoMes.reduce<PontoAcumulado[]>((acc, ponto) => {
      const saldo = calcularSaldoPonto(ponto)
      ...
      {pontosReverso.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          {pontosReverso.map((ponto) => {
            const tipo = TIPOS[ponto.tipo as keyof typeof TIPOS] || TIPOS.registro
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <SaldoBadge minutos={pontosComAcumulado.find(p => p.data === ponto.data)?.saldo ?? 0} formatter={minutosParaTexto} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'var(--space-1)' }}>
                  ...
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>acum. {minutosParaTexto(pontosComAcumulado.find(p => p.data === ponto.data)?.acumulado ?? 0)}</span>
                </div>
                {trabalhadas > 0 && (
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--text-sm)' }}>
                ...
                <span style={{ fontWeight: 500, color: 'var(--color-text)' }}>{TIPOS[detalhe.tipo as keyof typeof TIPOS]?.emoji} {TIPOS[detalhe.tipo as keyof typeof TIPOS]?.label}</span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--text-sm)', paddingTop: 'var(--space-2)', borderTop: '1px solid var(--color-divider)' }}>
                ...
                <SaldoBadge minutos={(detalhe as unknown as { saldo: number }).saldo} formatter={minutosParaTexto} />
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--text-sm)' }}>
                <span style={{ color: 'var(--color-text-muted)' }}>Acumulado</span>
                <SaldoBadge minutos={(detalhe as unknown as { acumulado: number }).acumulado} formatter={minutosParaTexto} />
              </div>
